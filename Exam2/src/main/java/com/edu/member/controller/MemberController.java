...
public class MemberController {
	...
	public String getMain() throws Exception {
		logger.info("MemberController getMain().....");
		return "/member/main"; 
	}
	...
	public String memberDelete(MemberDTO memberDTO, HttpSession session,
		...
		// 세션에 들어있는 memberDTO 정보를 가져와서 member변수에 저장한다.
	...
	public String postRegisterAjax(MemberDTO memberDTO) throws Exception {
		logger.info("MemberController getRegisterAjax POST.....");

		// 아이디입력란에 입력한 글자에 해당하는 아이디가 존재하면 1을,
		// 아이디가 존재하지 않으면 0을 돌려받는다.
		int result = memberService.idCheck(memberDTO);
		logger.info("MC------------- getRegisterAjax POST....." + memberDTO.toString());
